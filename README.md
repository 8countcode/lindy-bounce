...
- **Analyze your own bounce from video (on-device):** upload a side-view clip and the app tracks your pose with MediaPipe Pose Landmarker **entirely in the browser — the video is never uploaded**. It detects each bounce, measures depth / trunk-lean / knees-forward / tempo / left-right symmetry, maps them to the model to estimate a joint-protection score, and gives specific coaching cues. Includes a skeleton-overlay replay. (The pose engine + model are fetched once from a CDN on first use; only that generic model is downloaded, never your video.)
